...
public class GamePanel extends JPanel implements KeyListener, MouseListener{
	...
	// MASK
	private GameObject mask = components.get("mask");
	private boolean maskCaptured = false;
	private int updateCounter = 2500/12;
...
//------------------------------------ METHODS / HOW THE GAME WORKS ------------------------------------------------------------------//
	...
	public void drawBase(Graphics g) {
		...
		// ADJUST WHEN THE MASK IS CAPTURED
		if(maskCaptured) {
			if(Math.abs(baseCoords[0]) + Math.abs(baseCoords[1]) != 436) {
				if(baseCoords[0] < 0)
					baseCoords[1] = 436 + baseCoords[0];
				else
					baseCoords[0] = 436 + baseCoords[1];
			}
		}
		// MOVING BASE EFFECT
	...
	// DEPLOY MASK 
	private void maskHandler(Graphics g) {

		int acceleration = 2;
		
		// RESET EVERYTHING WHEN CHARACTER DIES
		if(!currentCharacter.isAlive()) {
			maskCaptured = false;
			if(pipes.get(0).getSpeed() < -2) {
				for(Pipe pipe: pipes) {
					pipe.setSpeed(pipe.getSpeed() + acceleration);
				}
			}
			if(baseSpeed > 2) {
		...
		if(!maskCaptured && currentCharacter.isAlive()) {
			...
			if(mask.getX() < 0-mask.getWidth()) {
				int pipeWidth = 78;
				int pointx = FlappyMaskMain.WIDTH + 5 + pipeWidth + random.nextInt(140);
				int pointy = random.nextInt(500-50) + 50;
				mask.setX(pointx);mask.setY(pointy);
				
			}
			else {
				// IF THE MASK IS STILL IN FRAME, MOVE THE MASK
				mask.setX(mask.getX()-2);
					
			}
				
			// CREATE RECTANGLE FOR MASK
			Rectangle maskRectangle = new Rectangle(mask.getX(), mask.getY(), mask.getWidth(), mask.getHeight());
			// CREATE RECTANGLE FOR CHARACTER
			Rectangle charRectangle = new Rectangle(currentCharacter.getX(), currentCharacter.getY(), 45, 32);
			if(maskRectangle.intersects(charRectangle)) {
				maskCaptured = true;
				mask.setX(FlappyMaskMain.WIDTH * 2 + 200);
			}
			else {
				// DRAW IF THE CHARACTER IS NOT TOUCHING THE MASK
				g.drawImage(mask.getObjectImage(), mask.getX(), mask.getY(), null);
			}
		}
		else if(currentCharacter.isAlive()){
			
			if(pipes.get(0).getSpeed() > -10 && baseSpeed < 10) {
				for(Pipe pipe: pipes) {
					pipe.setSpeed(pipe.getSpeed() - acceleration);
				}
				baseSpeed += 2;
		...
		for(Pipe pipe : pipes) {
			...
			if(currentCharacter.isAlive()) {
				if(pipe.collide(currentCharacter.getX(), currentCharacter.getY(), currentCharacter.CHARA_WIDTH, currentCharacter.CHARA_HEIGHT)) {
					...
					// KILL CHARACTER AND PLAY SOUND (DON'T KILL CHARACTER IF maskCaptured == true)
					if(!maskCaptured) {
